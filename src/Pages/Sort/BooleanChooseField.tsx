    ...
    <ul className="flex w-full gap-4 md:gap-8 ">
      {idValueDictionary.map((option, i: number) => {
        return (
          <li key={i}>
            <p className="mb-5"></p>
            <input type="radio" className="peer mb-5 hidden" name={fieldName} checked={form?.[fieldName] === String(option.value)} value={String(option.value)} id={option.id} onChange={handleChange} />
